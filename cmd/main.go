...
import (
	...
	"github.com/lonegunmanb/terraform-template-schema/v2/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh template schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-template-schema", options)
		if err != nil {
			return false, err
		}
